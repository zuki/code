...
レーダー上の輝点の作成はそのまま2次元で行い、Z軸は、ターゲットがプレーヤーのBoxComponentの
max.zより上の場合は+1、min.zより下の場合は-1、それ以外は0という3値とし、合わせて3次元とした。

上下の判断をしやすいように床を100下げ（もとは-100）、ターゲットのZ位置を変更した。

提供されている上下輝点のテクスチャが区別しずらい。

## 課題11.3

プレイヤーからArrowTargetに向かうベクトルとプレイヤーの前方ベクトルとの間の角を指示の通り求めると、ベクトルが右にあっても、
左にあっても[0, pi]となり、左右がわからない。そのため、左右の判定をして、左にある場合は角度を負にする処理を加えた。

左右の判定は、前方ベクトルを右に90度回転させたものとの内積が正なら右、負なら左とした。